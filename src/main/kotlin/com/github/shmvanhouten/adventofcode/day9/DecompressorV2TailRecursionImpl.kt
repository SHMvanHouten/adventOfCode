...
class DecompressorV2TailRecursionImpl {
    ...
    fun getDecompressionSize(compressedInput: String): Long = decompressTailRec(compressedInput)
    ...
    private tailrec fun decompressTailRec(compressedInput: String, factor: Int = 1, size: Long = 0): Long {
        ...
        if(indexOfMarker == -1) return (tempCompressedInput.length * factor).toLong()

        //if there are compressed chars in pos 0 add to size
        if(indexOfMarker > 0){
        ...
        //if there is marker in pos 0 and the marker doesn't span the entire input
        if(stringAfterMarkerAndNewFactor.first.length + (tempCompressedInput.indexOf(')') + 1) < tempCompressedInput.length){
    ...
    private fun processNonOverlappingMarkers(section: String, parentFactor: Int): Long {
        ...
        while(indexOfMarker != -1){
            if(indexOfMarker > 0){
                size += decompressTailRec(tempSection.substring(0, indexOfMarker))
                tempSection = tempSection.substring(indexOfMarker)
            }
            val markerSection = getMarkerSection(tempSection)
            size += decompressTailRec(markerSection)
