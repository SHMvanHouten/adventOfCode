...
class RoomKeyDecoderTest {
    ...
    @Test
    fun `it should assert a-b-c-d-e-f-g-h-987(abcde) is a valid room key`() {
        val key = "a-b-c-d-e-f-g-h-987[abcde]"
        val keyDecoder = RoomKeyDecoder()
        assertThat(keyDecoder.checkIfRoomIsReal(key), `is`(987))
    }

    @Test
    fun `it should assert a-b-c-d-e-f-g-h-987(acbde) is not a valid room key`() {
        val key = "a-b-c-d-e-f-g-h-987[acbde]"
        val keyDecoder = RoomKeyDecoder()
        assertThat(keyDecoder.checkIfRoomIsReal(key), `is`(0))
    }

    @Test
    fun `it should assert not-a-real-room-404(oarel) is a valid room key`() {
        val key = "not-a-real-room-404[oarel]"
        val keyDecoder = RoomKeyDecoder()
        assertThat(keyDecoder.checkIfRoomIsReal(key), `is`(404))
    }

    @Test
    fun `it should assert totally-real-room-200(decoy) is not a valid room key`() {
        val key = "totally-real-room-200[decoy]"
        val keyDecoder = RoomKeyDecoder()
        assertThat(keyDecoder.checkIfRoomIsReal(key), `is`(0))
    }
    ...
    @Test
    fun `it should sum up the ids of the real rooms`() {
        val keys = """aaaaa-bbb-z-y-x-123[abxyz]
            |bbbbb-aaa-z-y-x-123[abxyz]
        ...
        assertThat(keyDecoder.checkIfRoomsAreReal(keys), `is`(123 + 987 + 404))
    }
