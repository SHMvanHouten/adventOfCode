...
value 2 goes to bot 2"""
    ...
    @Test
    fun `it should solve the challenge input for values 17 and 61`() {
        val instructor = InstructionCentral()

        val botNumber = instructor.findBotThatComparesValues(day10ChallengeInput, 17, 61)
        assertThat(botNumber, equalTo(86))
    }

    @Test
    fun `it should calculate the chips in output 0 1 2 multiplied`() {
        val instructor = InstructionCentral()
        val instructions = """value 5 goes to bot 2
bot 2 gives low to bot 1 and high to bot 0
value 3 goes to bot 1
...
bot 0 gives low to output 2 and high to output 0
value 2 goes to bot 2"""
        val outputsMultiplied = instructor.findTheChipsInOutputsMultiplied(instructions, listOf<Int>(0, 1, 2))
        assertThat(outputsMultiplied, equalTo(30))
    }

    @Test
    fun `it should calculate the chips in output 0 1 2 multiplied for the challenge input`() {
        val instructor = InstructionCentral()

        val outputsMultiplied = instructor.findTheChipsInOutputsMultiplied(day10ChallengeInput, listOf<Int>(0, 1, 2))
        assertThat(outputsMultiplied, equalTo(22847))
    }
}
val day10ChallengeInput = """bot 123 gives low to bot 191 and high to bot 162
